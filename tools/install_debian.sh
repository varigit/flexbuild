...
UBOOT_WITH_SPL_IMAGE=u-boot-with-spl.imx
KERNEL_IMAGE=zImage
KERNEL_DTB=""
STORAGE_DEV=""
...
check_images()
{
	if [[ -n $UBOOT_WITH_SPL_IMAGE ]]; then
		if [[ ! -f $IMGS_PATH/$UBOOT_WITH_SPL_IMAGE ]] ; then
			red_bold_echo "ERROR: \"$IMGS_PATH/$UBOOT_WITH_SPL_IMAGE\" does not exist"
			exit 1
		fi
	else
...
install_bootloader_to_emmc()
{
	...
	if [[ -n $UBOOT_WITH_SPL_IMAGE ]]; then
		dd if=${IMGS_PATH}/${UBOOT_WITH_SPL_IMAGE} of=${node} bs=1K seek=1; sync
	else
...
if [[ $STORAGE_DEV == "nand" ]] ; then
	if [[ -n $UBOOT_WITH_SPL_IMAGE ]]; then
		red_bold_echo "ERROR: Installation on NAND flash device is not supported."
		exit 1
	fi
